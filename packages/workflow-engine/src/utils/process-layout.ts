/**
 * Layered layout of the block graph: the one geometry of the process the map draws in the browser
 * and the progress picture draws on the server. Browser-safe and pure: it imports nothing but the
 * bundled ELK engine, which runs in both.
 *
...
export interface LayoutTransition {
  to: string;
  label: string;
  /** Truthy for a transition back to an earlier block or to the block itself. */
  cycle?: { cause: string; exit: string } | boolean | null;
}

/** A block as the layout reads it: the map's `RunBlock` and the picture's visible block both fit. */
export interface LayoutBlock {
  id: string;
  /** Zero-based position in process order. */
  index: number;
  description: string;
  transitions: readonly LayoutTransition[];
}
...
export function transitionKey(from: string, transition: Pick<LayoutTransition, "to" | "label">) {
 ...
 */
export function rankSeparation(blocks: readonly LayoutBlock[], hubIds: readonly string[]): number {
...
function selfLoopExtent(block: LayoutBlock): number {
...
export interface LayoutBlocksOptions {
  ...
  /**
   * The size of every card, by block id. The map lets the estimate below stand for the card the
   * browser will draw; the progress picture measures its cards with its own text metric and
   * passes them in, so both go through the one layout with the sizes they draw.
   */
  sizes?: ReadonlyMap<string, { width: number; height: number }>;
}

/** The estimated size of every card: the map's width and the ported card's height estimate. */
export function estimatedBlockSizes(
  blocks: readonly LayoutBlock[],
...
export async function layoutBlocks(
  ...
  const hubs = new Set(hubIds);
  const indexOf = new Map(blocks.map((b) => [b.id, b.index]));
  const sizes = new Map(options.sizes ?? estimatedBlockSizes(blocks));
  for (const block of blocks) {
    if (!sizes.has(block.id)) throw new Error(`layoutBlocks: no size for block ${block.id}`);
  }

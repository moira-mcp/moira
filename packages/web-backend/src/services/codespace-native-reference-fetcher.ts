...
import type {
  ...
  CodespaceNativeReferenceResponse,
} from "@mcp-moira/shared";
import { publicIp } from "./github-codespaces-egress-proxy.js";
import { supportedCodespaceTransferMime } from "@mcp-moira/shared";
...
export class OpenAINativeReferenceFetcher implements CodespaceNativeReferenceFetcher {
  constructor(
  ...
  private async fetchUrl(url: URL, redirects: number): Promise<CodespaceNativeReferenceResponse> {
    if (
      url.protocol !== "https:" ||
    ...
    return new Promise((resolve, reject) => {
      const options: RequestOptions = {
        ...
        method: "GET",
        headers: { accept: "*/*", "user-agent": "Moira-Codespace-Transfer/1" },
      ...
      const request = this.requestImpl(options, (response) => {
        ...
        if (
          ...
          !supportedCodespaceTransferMime(contentType.toLowerCase().split(";", 1)[0].trim())
        ) {
          response.destroy();

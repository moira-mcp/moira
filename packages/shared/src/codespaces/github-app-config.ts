import {
  getAppPrefix,
  getCodespaceCredentialVaultKey,
  getCodespaceCredentialVaultKeyVersion,
  getCodespaceGitHubAppCallbackUrl,
  getCodespaceGitHubAppInstallUrl,
  getCodespaceGitHubAppClientId,
  getCodespaceGitHubAppClientSecret,
} from "../config/env.js";
import { getBaseUrl } from "../config/urls.js";

export const GITHUB_CODESPACE_CALLBACK_PATH = "/api/integrations/github/callback";
export const GITHUB_SETTINGS_FRAGMENT = "integrations-github";

export type CodespaceGitHubConfigReason =
  | "NOT_CONFIGURED"
...
export type CodespaceGitHubConfigStatus =
  | { state: "disabled"; reason: "NOT_CONFIGURED"; settingsUrl: string }
  | { state: "invalid"; reason: CodespaceGitHubConfigReason; settingsUrl: string }
  | {
      state: "available";
      clientId: string;
      clientSecret: string;
...
export interface CodespaceGitHubConfigInput {
  clientId?: string;
  clientSecret?: string;
  callbackUrl?: string;
  installationUrl?: string;
...
export function evaluateCodespaceGitHubConfig(
  input: CodespaceGitHubConfigInput,
): CodespaceGitHubConfigStatus {
  ...
  if (
    ...
    parsedCallback.hash ||
    parsedCallback.pathname !== GITHUB_CODESPACE_CALLBACK_PATH ||
...
export function getCodespaceGitHubConfig(): CodespaceGitHubConfigStatus {
  return evaluateCodespaceGitHubConfig({
    clientId: getCodespaceGitHubAppClientId(),
    clientSecret: getCodespaceGitHubAppClientSecret(),
    callbackUrl: getCodespaceGitHubAppCallbackUrl(),
    installationUrl: getCodespaceGitHubAppInstallUrl(),
    vaultKeyHex: getCodespaceCredentialVaultKey(),
    vaultKeyVersion: getCodespaceCredentialVaultKeyVersion(),
    baseUrl: getBaseUrl(),

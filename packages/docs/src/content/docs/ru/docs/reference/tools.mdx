...
- `list` - Ваша библиотека workflow (own + added + shared)
...
- `marketplace` - Поиск, добавление, публикация, оценка и шеринг workflow
...
Возвращает **вашу библиотеку** — не весь публичный каталог. Элементы приходят из трёх источников:

- **own** — флоу, которые вы создали
- **added** — флоу из маркетплейса, которые вы добавили (через `marketplace add`), а также
  курируемые официальные базовые флоу (quick-task, robust-task, …), засеянные в каждый аккаунт
- **shared** — приватные флоу, которыми с вами поделились по ссылке

Метка **official** основана на происхождении (provenance): флоу официален только если он
из официального каталога Moira (аккаунт `moira`), независимо от источника — засеянные
базовые флоу являются official-записями origin `added`. Ваши собственные флоу (и, на
self-host инстансе, флоу оператора) **не** официальны. Отфильтровать официальные —
`source: "official"`. Каждый элемент библиотеки также несёт `version`, чтобы различать
похожие копии.

Прочие публичные флоу маркетплейса **не** попадают в `list()`, пока вы их не добавите —
для просмотра магазина используйте инструмент [`marketplace`](#marketplace). Это держит
рабочий набор агента небольшим и осмысленным по мере роста каталога.

:::note[Ломающее изменение]
Раньше `list()` возвращал все доступные публичные workflow. Теперь он возвращает
библиотеку. Поскольку список MCP-инструментов кэшируется во время авторизации, клиентам
нужно переподключиться, чтобы получить новое поведение и новый инструмент `marketplace`.
:::

**Параметры:**

- `source` (string, опционально) - Фильтр библиотеки: `all` | `official` | `added` | `mine` | `shared` (по умолчанию: `all`)
- `search` (string, опционально) - Фильтр по имени workflow (подстрока)
- `limit` / `offset` (number, опционально) - Пагинация (по умолчанию вся библиотека)

**Возвращает:**

- `workflows` - массив `{ id, workflowId, slug, name, version, description, origin, kind? }`. Передайте `id` результата (ссылку «handle/slug») в `start()`.
- `total` - число совпавших записей библиотеки

**Пример:**

```json
list({ source: "added" })
→ { workflows: [{ id: "alice/data-pipeline", name: "Data Pipeline", origin: "added", kind: "reference" }], total: 1 }
```
...
Поиск, добавление, публикация, оценка и шеринг workflow. Маркетплейс управляется агентом —
всё, что делает веб-интерфейс, доступно и как MCP-действие, поэтому агент может
просматривать и добавлять флоу без участия человека (без копирования команд).

Действия магазина (`search` / `info` / `add` / `publish` / `rate` / `share`) требуют,
чтобы маркетплейс был включён на инстансе; где он выключен — они возвращают понятное
сообщение «маркетплейс не включён», но локальная библиотека (`list`) продолжает работать.

**Параметр:** `action` (string, обязательный) выбирает операцию.

| Действие    | Входные данные                                                               | Возвращает                                                                                             |
| ----------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| `search`    | `q`, `category`, `tags`, `sort` (recent\|rating\|installs\|trending), `page` | карточки магазина, у каждой `ref` ("handle/slug")                                                      |
| `info`      | `ref`                                                                        | полные детали (описание, рейтинги, автор, число нод, entitlement); записывает просмотр                 |
| `add`       | `ref`, `fork` (true = редактируемая копия; по умолчанию = живая ссылка)      | добавляет флоу в библиотеку; он появляется в `list()` (нельзя добавить свой собственный листинг)       |
| `remove`    | `ref`                                                                        | origin-aware: отвязывает добавленную ссылку (оригинал не трогается), но УДАЛЯЕТ флоу, которым владеете |
| `publish`   | `workflowId`, `category`, `tags`, `summary`                                  | делает ваш workflow публичным (listed). Платные поля отклоняются, пока продажа выключена               |
| `unpublish` | `workflowId`                                                                 | делает ваш опубликованный workflow снова приватным                                                     |
| `rate`      | `ref`, `stars` (1-5), `review`                                               | сохраняет вашу оценку/отзыв (нельзя оценивать свой флоу)                                               |
| `share`     | `workflowId`, `userHandle?`                                                  | приватный шеринг — выдать доступ пользователю, либо без `userHandle` сгенерировать ссылку-приглашение  |

Удаление из библиотеки учитывает происхождение (origin-aware) и сообщает, что именно
сделано: удаление _добавленной_ ссылки отвязывает её (оригинал не трогается); удаление
флоу, которым вы _владеете_ (включая импортированные или форкнутые копии), удаляет его
(и снимает с публикации, если он был опубликован); удаление флоу, которым с вами
_поделились_, отзывает ваш доступ. Удаление никогда не сообщает об успехе, не удалив ничего.

**Типичный сценарий агента:**

```json
marketplace({ action: "search", q: "data pipeline" })
marketplace({ action: "info", ref: "alice/data-pipeline" })
marketplace({ action: "add", ref: "alice/data-pipeline" })
list({ source: "added" })          // флоу теперь здесь
start({ workflowId: "alice/data-pipeline", parentExecutionId: "none" })
```
...
Редактирование сохраняет текущую видимость потока: редактирование опубликованного
(публичного, листингованного) потока **не** снимает его с публикации — листинг
продолжает отдавать обновлённую версию. Чтобы сделать поток приватным, снимите его с
публикации явно (`marketplace` `unpublish`) или используйте `set-visibility`.
...
Меняет видимость workflow (public/private).

**Параметры:**

- `action`: `"set-visibility"` (обязательный)
- `workflowId` (string, обязательный) - ID workflow
- `visibility` (string, обязательный) - "public" или "private"

**Возвращает:**

- Предыдущую и новую видимость

Установка `private` для потока с активным листингом в маркетплейсе отклоняется (поток
остаётся публичным и листингованным). Сначала снимите его с публикации через `marketplace`
`unpublish` — это делает поток приватным и снимает листинг за один шаг.

...
- `list` - Ваша библиотека workflow (core + own + added + shared)
...
Возвращает **вашу библиотеку** — не весь публичный каталог. Библиотека состоит из:

- **core** — официальные встроенные флоу (quick-task, robust-task, …), доступны всем
- **own** — флоу, которые вы создали
- **added** — флоу из маркетплейса, которые вы добавили (через `marketplace add`)
- **shared** — приватные флоу, которыми с вами поделились по ссылке

Публичные флоу маркетплейса **не** попадают в `list()`, пока вы их не добавите —
...
- `source` (string, опционально) - Ограничить одним источником: `core` | `own` | `added` | `shared` (по умолчанию: все)
...
- `workflows` - массив `{ id, workflowId, slug, name, version, description, origin, kind? }`. Передайте `id` результата ("handle/slug", иначе id workflow) в `start()`.

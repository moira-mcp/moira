...
Карта прогресса запуска выводится из исполняемого графа: каждая нода принадлежит одному этапу, каждая связь, выходящая из этапа, подписана, а каждый возврат объясняет причину и условие выхода. Этап реализации привязан к счётчикам плана — `current_step_index` как выполняемый unit и `total_steps` как их число, — поэтому данные прогресса запуска (`session progress` и API прогресса выполнения) сообщают, сколько units плана завершено из скольких и сколько занял каждый проход через этап.

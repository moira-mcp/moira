...
nodes contain only `id`, template-enabled `label`, and an optional static `connections.default` used
for drawing. User-visible waiting nodes in the primary graph select a milestone with
`progressNodeId`.
...
milestones complete only after the graph reaches its end with no current node. Cancellation retains
the earlier current node and therefore preserves its last-known index-derived state.

The engine also exposes one shared horizontal visual model and a bounded light/dark PNG renderer.
...
On execution pages this same model appears as an always-visible horizontal strip above the technical
graph and detail tabs. It centers the current milestone, scrolls on narrow screens, and selecting a
milestone focuses its mapped node in the technical workflow graph. Workflows without progress keep
the standard inspector unchanged.

...
- `list` - List your workflow library (own + added + shared)
...
- `marketplace` - Search, adopt, publish, rate, and share workflows
...
Returns **your library** — not the whole public catalog. Items come from three origins:

- **own** — workflows you created
- **added** — marketplace flows you adopted (via `marketplace add`), plus the curated
  official base flows (quick-task, robust-task, …) seeded into every account
- **shared** — private flows shared with you by link

The **official** flag is provenance-based: a flow is official only when it comes from the
official Moira catalog account (`moira`), regardless of origin — the seeded base flows are
official `added` entries. Your own flows (and, on a self-host instance, the operator's
flows) are **not** official. Filter to the official set with `source: "official"`. Each
library item also carries a `version` so you can tell otherwise-identical copies apart.

The marketplace's other public flows are **not** in `list()` until you add them — browse
the store with the [`marketplace`](#marketplace) tool. This keeps the agent's working set
small and intentional as the catalog grows.

:::note[Breaking change]
`list()` previously returned all accessible public workflows. It now returns the library.
Because the MCP tool set is cached at authorization time, clients must reconnect to pick
up the new behavior and the new `marketplace` tool.
:::

**Parameters:**

- `source` (string, optional) - Filter the library: `all` | `official` | `added` | `mine` | `shared` (default: `all`)
- `search` (string, optional) - Filter by workflow name (substring)
- `limit` / `offset` (number, optional) - Pagination (default: the whole library)

**Returns:**

- `workflows` - array of `{ id, workflowId, slug, name, version, description, origin, kind? }`. Pass a result's `id` (a "handle/slug" reference) to `start()`.
- `total` - the number of library entries matched
...
Discover, adopt, publish, rate, and share workflows. The marketplace is agent-operated —
everything the web UI does is also an MCP action, so the agent can browse and adopt flows
headless (no copy-pasting commands).

Store actions (`search` / `info` / `add` / `publish` / `rate` / `share`) require the
marketplace to be enabled on the instance; where it is off they return a clear
"marketplace not enabled" message, but your local library (`list`) still works.

**Parameter:** `action` (string, required) selects the operation.

| Action      | Inputs                                                                       | Returns                                                                                                 |
| ----------- | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `search`    | `q`, `category`, `tags`, `sort` (recent\|rating\|installs\|trending), `page` | store cards, each with a `ref` ("handle/slug")                                                          |
| `info`      | `ref`                                                                        | full detail (description, ratings, author, node count, entitlement); records a view                     |
| `add`       | `ref`, `fork` (true = editable copy; default = live reference)               | adopts the flow into your library; it then appears in `list()` (you cannot adopt your own listing)      |
| `remove`    | `ref`                                                                        | origin-aware: un-adopts an added reference (original untouched), but DELETES a flow you own             |
| `publish`   | `workflowId`, `category`, `tags`, `summary`                                  | makes your workflow public (listed). Paid fields are rejected while selling is off                      |
| `unpublish` | `workflowId`                                                                 | makes your listed workflow private again                                                                |
| `rate`      | `ref`, `stars` (1-5), `review`                                               | upserts your rating/review (you cannot rate your own flow)                                              |
| `share`     | `workflowId`, `userHandle?`                                                  | privately shares your workflow — grant a user directly, or omit `userHandle` to generate an invite link |

Library removal is **origin-aware** and reports what it did: removing an _added_ reference
un-adopts it (the original is untouched); removing a flow you _own_ (including imported or
forked copies) deletes it (and unlists it if it was published); removing a flow _shared_ with
you revokes your access. Removal never reports success without actually removing anything.
...
list({ source: "added" })          // the flow now appears here
...
Editing keeps the flow's current visibility: editing a published (public, listed) flow
does **not** unpublish it — the listing keeps serving the updated version. To take a flow
private, unpublish it explicitly (`marketplace` `unpublish`) or use `set-visibility`.
...
Setting `private` on a flow that has an active marketplace listing is rejected (the flow
stays public and listed). Unpublish it first via `marketplace` `unpublish`, which makes it
private and unlists it in one step.

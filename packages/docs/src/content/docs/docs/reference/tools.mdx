...
timeout (`timeout_seconds`, default 300) and optional per-stream output limits, and returns bounded `stdout`, `stderr` and the exact
...
seconds and a background command its whole ceiling. Stop either kind by calling the tool

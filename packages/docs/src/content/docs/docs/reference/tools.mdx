...
`exit_code`. Supply at most one stdin form: `stdin_text` for UTF-8 text or `stdin_file` for a native
...
`workspace_read` returns UTF-8 text with `offset`, `total_size` and `sha256`; `length` defaults to

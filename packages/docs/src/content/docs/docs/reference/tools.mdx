...
keep `operation_id` and the available output. A result whose `state` is not terminal is not a
success: call the same tool again with only `workspace_id` and `operation_id` to obtain the stored
outcome without running the command or mutation twice.

...
- `list` - List your workflow library (core + own + added + shared)
...
Returns **your library** — not the whole public catalog. The library is:
...
The marketplace's public flows are **not** in `list()` until you add them — browse the
store with the [`marketplace`](#marketplace) tool. This keeps the agent's working set
...
- `source` (string, optional) - Restrict to one origin: `core` | `own` | `added` | `shared` (default: all)
...
- `workflows` - array of `{ id, workflowId, slug, name, version, description, origin, kind? }`. Pass a result's `id` ("handle/slug" when known, else the workflow id) to `start()`.

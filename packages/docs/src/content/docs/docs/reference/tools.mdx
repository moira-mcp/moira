...
timeout and optional per-stream output limits, and returns bounded `stdout`, `stderr` and the exact
...
`workspace_read` returns UTF-8 text with `offset`, `total_size` and `sha256`; a non-text range
returns `WORKSPACE_BINARY_READ_REQUIRES_DOWNLOAD`. `workspace_write` replaces a file atomically

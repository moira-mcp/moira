...
Searches nodes by text in directive, completionCondition, or ID. Use `|` to provide bounded literal alternatives; other regex metacharacters are matched literally.

**Parameters:**

- `action`: `"search-nodes"` (required)
- `workflowId` (string, required) - Workflow ID
- `query` (string, required) - Search text; separate bounded literal alternatives with `|`

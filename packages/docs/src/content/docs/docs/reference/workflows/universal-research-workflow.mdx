...
```bash
mcp__moira__start({ action: "prepare", workflowId: "moira/universal-research-workflow", parentExecutionId: "none" })
...
- separate explicit authority for static publication and user notification.
...
- `published_and_notified` additionally requires separate notification authority and an observed successful send through a configured user channel.
- Publication unavailable, unauthorized, or failed and notification unauthorized, unsent, or errored are distinct terminal outcomes that preserve the accepted local result.

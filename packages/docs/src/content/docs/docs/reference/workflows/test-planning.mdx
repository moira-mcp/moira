...
Agent directives use named `success` connections, and the review gate is a condition node whose ordered `cases` select an output, with `default` taken when no case holds:

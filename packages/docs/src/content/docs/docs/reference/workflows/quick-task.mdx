---
title: Quick Task
description: Filesystem-backed bounded non-development planning, execution, independent review, and user acceptance
---
...
<Aside type="tip">
Use Quick Task when an agent has filesystem access and bounded non-development work fits a plan of at most ten meaningful units. For software implementation, bug fixing, refactoring, migrations, or rollout, use [Software Development Flow Lite](./software-development-flow-lite/) for one contained low-risk result or the [full Software Development Flow](./software-development-flow/) when risk, uncertainty, or spread grows. Use [Robust Task](/docs/reference/workflows/robust-task/) when a complex general task needs a stronger recovery policy for partial execution or operational failures.
</Aside>
...
The execution card and exported progress image use the same five-stage presentation: Scope → Plan → Execute → Verify → Result. The execution note remains the task title. Every user-visible waiting responsibility, including the jump-only replan entry, maps to one of these stages; the presentation does not add routes or change the executable graph.

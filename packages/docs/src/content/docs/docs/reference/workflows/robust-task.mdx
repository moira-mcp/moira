...
The Execute stage binds the plan's counters — `current_step` as the step in progress and `total_steps` as the total — so the run reports how far through the plan it is; once the route has reached that stage, a user notification carries the same count in its footer.

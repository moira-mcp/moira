...
`skipNotificationCheck: true` is required on an unconfigured account because the workflow contains its own legacy `telegram-notification` test node and would otherwise trigger ordinary-notification preflight. `skipTelegramCheck` remains a deprecated alias with the same behavior.

...
Set `skipNotificationCheck: true` during prepare on an unconfigured account because the workflow contains its own legacy `telegram-notification` test node and execute would otherwise return `START_PRECONDITION_CHANGED` at ordinary-notification preflight. `skipTelegramCheck` remains a deprecated alias with the same behavior.

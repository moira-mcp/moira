...
3. **Update** (re-import): when the source flow is updated, download it again and import
   the new file. Because the portable file records where it came from, the import updates
   your existing copy in place (graph and version refreshed) instead of creating a
   duplicate.

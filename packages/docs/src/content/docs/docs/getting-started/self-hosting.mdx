...
`--force`, MCP, an HTTP API, or UI transport. Follow the current image's startup log for whether the
container remains available or must be started again after recovery.

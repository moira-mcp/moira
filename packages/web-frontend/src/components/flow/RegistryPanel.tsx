/**
 * The variable registry of the definition: every declared global with its type, description
 * and default. In edit mode each entry is editable (the default as JSON, validated locally
 * before it is applied), entries can be removed and new ones declared. The server remains the
 * judge: the save validates the whole definition.
 */
...
function Entry({ name, entry }: { name: string; entry: RegistryVariable }): React.JSX.Element {
  ...
  return (
    <li
      className="space-y-1.5 rounded-lg border bg-card p-2.5 text-sm"
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        ...
        {enabled ? (
          <Select
            value={entry.type}
            onValueChange={(type) =>
              setRegistry(name, { ...entry, type: type as RegistryVariable["type"] })
            }
          >
            <SelectTrigger
          ...
          <span className="rounded border bg-muted/40 px-1.5 text-[11px]">{entry.type}</span>
        )}
        {enabled && (
          <button
            type="button"
      ...
      <div className="flex items-baseline gap-2 text-xs">
        <span className="shrink-0 text-muted-foreground">
          {t("pages.flowPage.registry.default")}
        </span>
        {enabled ? (
          <span className="min-w-0 flex-1 space-y-0.5">
            ...
            />
            {invalid && (
              <span className="block text-[11px] text-destructive" data-testid="registry-invalid">
          ...
          <code className="min-w-0 break-all font-mono">
            {entry.default === undefined ? (
              <span className="text-muted-foreground">
                {t("pages.flowPage.registry.noDefault")}
              </span>
            ) : (
      ...
      {enabled && (
        <label className="block space-y-1">
          ...
          <Textarea
            ...
            className={cn("min-h-[96px] font-mono text-xs", schemaInvalid && "border-destructive")}

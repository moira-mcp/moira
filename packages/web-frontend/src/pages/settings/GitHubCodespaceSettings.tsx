...
export const GitHubCodespaceSettings: React.FC = () => {
  ...
  const disconnect = async () => {
    try {
      ...
      const next = await apiClient.disconnectGitHubCodespace();
      setStatus(next);
  ...
  const refreshRepositories = async () => {
    try {
      ...
      setStatus(next);
      if (next.repositoriesStale) {
        toast.warning(t("pages.settings.github.repositoriesStale"));
      } else {
        toast.success(t("pages.settings.github.repositoriesRefreshed"));
      }
    } catch {
    ...
    <Card data-testid="github-codespace-settings">
      <CardHeader className="gap-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Badge variant={statusVariant(status.state)} data-testid="github-codespace-status">

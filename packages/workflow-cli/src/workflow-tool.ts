 ...
 *   search <text>                    Search nodes (bounded RE2 regex supported)
...
${c("cyan", "Examples:")}
  ...
  moira-workflow dev-flow.json update analyze-and-plan --directive "новый текст"
  moira-workflow dev-flow.json search "development-plan.md"
...
`);
  ...
  if (args[0] === "create") {
    // moira-workflow create <output-file> --name <name>
    command = "create";
    file = args[1]; // output file path
    nodeId = undefined;
  } else {
    // Normal case: moira-workflow <file> <command> [args]
    file = args[0];
    command = args[1];
    nodeId = args[2];
...
async function main(): Promise<void> {
  ...
  if (config.command === "create") {
    ...
    if (nameIdx === -1 || !args[nameIdx + 1]) {
      ...
      console.log("Usage: moira-workflow create <file> --name <name>");
      process.exit(1);
    }
  ...
  switch (config.command) {
    ...
    case "diff":
      if (!config.nodeId) {
        ...
        console.log("Usage: moira-workflow <file1> diff <file2>");
        process.exit(1);
      }
    ...
    case "copy": {
      if (!config.nodeId) {
        ...
        console.log("Usage: moira-workflow <source-file> copy <dest-file> [--name <name>]");
        process.exit(1);
      }

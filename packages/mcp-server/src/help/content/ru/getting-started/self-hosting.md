...
GitHub**, нажимает **Подключить GitHub** и один раз авторизуется в GitHub. Если приложение ещё не
...
Теми же кодспейсами пользователи управляют в **Настройки → Интеграции → Облачные кодспейсы**:
создают кодспейс для одобренного репозитория, запускают или останавливают его
(остановка сохраняет данные репозитория) и удаляют после явного подтверждения. Простаивающие
кодспейсы каждого пользователя приостанавливаются сами: настройки `codespaces.auto_stop_enabled`
(по умолчанию включена) и `codespaces.idle_timeout_minutes` (по умолчанию 30, от 5 до 240) на
странице **Настройки** определяют, когда Moira останавливает кодспейс, которым ни один агент не
пользовался через Moira, а следующая операция агента снова его запускает. Прямая работа в браузере,

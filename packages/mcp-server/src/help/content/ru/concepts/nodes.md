...
Ветвление выполнения в один из нескольких выходов по упорядоченным case.
...
| `expressions`              | Нет         | Арифметические выражения, вычисляемые после валидации ответа       |
| `cases`                    | Нет         | Упорядоченные case маршрутизации по проверенному ответу узла       |
| `connections.success`      | Да          | Выход по умолчанию: берётся, когда ни один case не сработал        |
| `connections.error`        | Нет         | Управляющий выход при сбое выражения на узле                       |
| `connections.timeout`      | Нет         | Зарезервированный управляющий выход; кейс не может его называть    |
...
У этого цикла валидации нет ограничения или перенаправления на уровне узла; бизнес-логику retry и
эскалации нужно явно моделировать в графе workflow.

### Маршрутизация по ответу

Узел agent-directive может сам решать, куда продолжится выполнение, исходя из своего проверенного
ответа. `cases` содержит упорядоченные записи `{ "when": <структурированное условие>, "output":
"<ключ соединения>" }`; первое сработавшее `when` выбирает свой выход, а если не сработало ни одно,
узел берёт `success`. Условия читают контекст уже после того, как ответ влит в него, поэтому поле
ответа доступно по краткому имени (`review_outcome` для объявленной глобальной переменной) и по
локальному пути узла (`review.review_outcome`).

```json
{
  "id": "review",
  "type": "agent-directive",
  "directive": "Проверь изменение",
  "completionCondition": "Вердикт зафиксирован",
  "inputSchema": {
    "type": "object",
...
`error` и `timeout` — зарезервированные управляющие выходы: case не может их называть, и отдельный
case для них не нужен.

## Condition Node

Ветвление выполнения по упорядоченным case. Каждый case связывает структурированное условие с
ключом соединения, который он выбирает; побеждает первый case, чьё `when` сработало, а если не
сработало ни одно, узел берёт `default`. Решение с двумя исходами — это один case плюс `default`:
...
Любое число исходов записывается так же — по одному case на каждый авторский выход:
...
| Свойство              | Обязательно | Описание                                              |
| --------------------- | ----------- | ----------------------------------------------------- |
| `cases`               | Да          | Упорядоченные записи `{ when, output }`; минимум одна |
| `cases[].when`        | Да          | Объект структурированного условия                     |
| `cases[].output`      | Да          | Ключ соединения, который выбирает этот case           |
| `expressions`         | Нет         | Арифметические выражения, вычисляемые до case         |
| `connections.default` | Да          | Выход, когда ни один case не сработал                 |
| `connections.error`   | Нет         | Управляющий выход при сбое выражения на узле          |

Любой другой ключ `connections` — авторский выход, названный в одном из case. `error` зарезервирован
для управления потоком: case не может его называть, и отдельный case для него не нужен.

### Выражения на узлах маршрутизации

И `condition`, и `agent-directive` принимают `expressions` — тот же песочный арифметический
интерпретатор и та же проверка по реестру, что и у отдельного [узла expression](#expression-node).
Присваивания должны обращаться к объявленным глобальным переменным. Выражения выполняются до case
(на узле agent-directive — после валидации его ответа), поэтому case может прочитать вычисленное, а
присваивания публикуются только при успехе узла. Сбой выражения ведёт в `connections.error`, если он
есть у узла; иначе узел падает — ровно как отдельный узел expression.
...
Живой каталог типов отличает недоступный runner от доступного реестра, в котором не установлено
конкретное расширение. Контракты манифеста, SDK и исполнения описаны в [Написании

...
содержимое в контекст агента. Сервер создаёт пятиминутное разрешение на tar-загрузку, привязанное к
ноде; агент выполняет или повторяет точную команду из сгенерированной директивы и завершает шаг
значением `null` или `{}`.

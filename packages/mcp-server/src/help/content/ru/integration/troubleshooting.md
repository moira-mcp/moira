...
**Причина:** Попытка больше не соответствует текущему предъявлению execution и была отклонена до
работы обработчика.
...
**Причина:** Попытка уже связана с другими входными данными, поэтому новая отправка отклонена до
работы обработчика.
...
**Причина:** Сохранённая живая попытка относится к другой ноде или версии workflow и не может быть
безопасно перепривязана к текущему execution.

**Решение:** Не повторяйте старую попытку. Изучите execution и текущее определение workflow; здесь
нужно явное исправление или осознанный перезапуск, а не автоматический повтор.

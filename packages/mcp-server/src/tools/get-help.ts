...
function getTopicList(helpDirectory = HELP_CONTENT_DIR): string {
  ...
  result += `| one bounded external-source question needing durable evidence, independent semantic review bounded by a repair budget, pre-access source-authority review, optional work without a writable workspace, and truthful complete or limited local delivery | \`moira/verified-research\` |\n`;
  ...
  result += `| evidence-linked product selection with current prices/terms, profile-specific recommendations, buyer guidance, reviewed self-contained HTML, and optional delivery; start with \`skipNotificationCheck: true\` when no ordinary channel is configured | \`moira/smart-purchase-assistant\` |\n`;
  ...
  result += `| build a verified screencast video from source-backed facts and story, local materials, a free draft, chosen narration, and permission-scoped handoff | \`moira/agentic-screencast-video\` |\n\n`;

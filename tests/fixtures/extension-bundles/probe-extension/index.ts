...
const behave = defineNode({
  type: "probe.behave",
  // Schemas live in the manifest; restating them here is optional and would be checked against it.
  async handler({ config, signal, services }) {
    const behaviour = String(config.behaviour ?? "succeed");
    services.log(`probe started ${behaviour}`);
    ...
    switch (behaviour) {
      ...
      case "sleep":
        // Finishes normally, but slowly enough to still be running when another call's deadline
        // passes — the case that shows whose deadline ended whose call.
        await new Promise((resolve) => setTimeout(resolve, Number(config.ms ?? 500)));

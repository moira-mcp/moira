...
const completion = {
  ...
  content_summary: "The reviewed text deliverable supports the stated audience use.",
...
function terminal(status: "complete" | "limited" | "blocked" | "aborted"): MockInput {
  return ({ executionId }) => ({
    artifact_path: `./moira-ws/content-creation-${executionId}/final-report.md`,
...
function inputs(overrides: Record<string, MockInput> = {}): Record<string, MockInput> {
  return {
    intake: {
      ...
      content_goal: "Launch the authorized offer to the stated audience.",
      content_use: "Generate qualified demo requests.",
      content_scope: ["Landing page", "Email"],
    },
    "initialize-contract": { contract_outcome: "ready" },
    "frame-brief": { brief_outcome: "ready", revision_request: sentinel },
    "build-evidence": { evidence_status: "ready", revision_request: sentinel },
    "create-content": { package_outcome: "ready", revision_request: sentinel },
    "complete-content": completion,
    "validate-package": { validation_outcome: "pass" },
    "semantic-review": { review_outcome: "pass" },
    "repair-content-validation": changed,
    "repair-content-semantic": changed,
    "repair-evidence-validation": changed,
    "repair-evidence-semantic": changed,
    "repair-brief-validation": changed,
    "repair-brief-semantic": changed,
    "reassess-contract": {
    ...
    "publish-content": {
      ...
      public_url: "https://example.test/content",
    },
...
describe("content-creation", () => {
  ...
  test.each([
    ["brief replan without active cause", "frame-brief", { brief_outcome: "replan" }, {}],
  ...
  test("rejects the neutral sentinel as a teleport cause", async () => {
    const result = await run({
      ...
      teleportAfter: { afterNode: "frame-brief", teleportTo: "revise-process" },
      expect: { status: "failed" },
    });
    expect(result.status).toBe("failed");
      ...
      },
      {
        scenario: {
          ...
          mockInputs: inputs({
            ...
            "complete-content": { ...completion, result_status: "limited" },
          }),
        ...
        scenario: {
          ...
          mockInputs: inputs({
            intake: {
              ...
              publication_target: "Authorized site/account",
              notification_authority: "authorized",
            },
          }),
          expect: { status: "completed", reaches: ["publish-content", "send-notification", "end"] },
        },
      },
      {
        scenario: {
          name: "declined effects",
          mockInputs: inputs({
            intake: {
              intake_outcome: "ready",
              operating_mode: "autonomous",
          ...
          expect: {
            status: "completed",
            reaches: ["set-publication-declined", "set-notification-declined", "end"],
          },
        },
      },
    );

    for (const owner of ["content", "evidence", "brief"] as const) {
      for (const source of ["validation", "semantic"] as const) {
        ...
        cases.push({
          scenario: {
            ...
            expect: { status: "completed", reaches: [repair, "complete-content", "end"] },
          },
        });
      }
    }

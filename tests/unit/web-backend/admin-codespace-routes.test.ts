...
import {
  createAdminCodespaceRoutes,
  type AdminCodespaceServices,
} from "../../../packages/web-backend/src/routes/admin-codespaces.js";
...
function appWith(services: AdminCodespaceServices) {
  const app = express();
  ...
  app.use("/api/admin/codespaces", createAdminCodespaceRoutes(services));
  app.use(setupErrorMiddleware());
  return app;
}

describe("admin codespace control routes", () => {
  ...
  test("validates the control body before touching the service", async () => {
    const setControl = jest.fn<CodespaceResourceService["setControl"]>();
    const app = appWith({
    ...
    for (const [scope, body] of [
      ...
      const response = await request(app).put(`/api/admin/codespaces/controls/${scope}`).send(body);
  ...
  test("forwards a disable request with the administrator identity and trimmed reason", async () => {
    ...
    const response = await request(
      ...
      .put("/api/admin/codespaces/controls/global")
  ...
  test("fails closed when the provider composition is absent or the scope is foreign", async () => {
    ...
    expect(absent.body.error.code).toBe("CODESPACE_NOT_CONFIGURED");
      ...
      appWith({
        ...
        resource: {
          ...
          setControl: jest.fn(async () => {
            throw new CodespaceResourceError("CODESPACE_RESOURCE_INVALID", "scope not managed");
          }),
        },
      }),
    )
      .put("/api/admin/codespaces/controls/provider:other-cloud")
      .send({ disabled: true });
    expect(foreign.status).toBe(400);
    expect(foreign.body.error.code).toBe("CODESPACE_RESOURCE_INVALID");
  });
});

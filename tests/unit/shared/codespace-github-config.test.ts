...
describe("GitHub codespace configuration", () => {
  test("keeps the feature disabled when no codespace secret is configured", () => {
    expect(
      evaluateCodespaceGitHubConfig({ baseUrl: valid.baseUrl, appPrefix: valid.appPrefix }),
    ).toEqual({
      state: "disabled",
  ...
  test("accepts an exact same-origin callback and dedicated strong vault key", () => {
    expect(evaluateCodespaceGitHubConfig(valid)).toEqual({
      state: "available",
  ...
  test.each([
    ...
    expect(evaluateCodespaceGitHubConfig(input)).toMatchObject({ state: "invalid", reason });
  });

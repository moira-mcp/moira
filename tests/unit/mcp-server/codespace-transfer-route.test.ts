...
import { createCodespaceTransferDownloadHandler } from "../../../packages/mcp-server/src/codespace-transfer-route.js";
import type { CodespaceTransferRecord, CodespaceTransferService } from "@mcp-moira/shared";
...
const record = {
  ...
  createdAt: Date.now(),
  updatedAt: Date.now(),
} satisfies CodespaceTransferRecord;

function app(service: Pick<CodespaceTransferService, "claimDownload" | "consume">) {
  const value = express();
  value.get("/files/:token", createCodespaceTransferDownloadHandler(service));
  return value;
}

describe("codespace transfer download route", () => {
  test("streams one private object with no-store, noindex and attachment protections", async () => {
    const service = {
      claimDownload: jest.fn<CodespaceTransferService["claimDownload"]>(async () => ({
        record,
        stream: Readable.from([Buffer.from([0, 255, 1, 2])]),
      })),
      consume: jest.fn<CodespaceTransferService["consume"]>(async () => undefined),
    };
  ...
  test("reveals no transfer metadata for an invalid or consumed capability", async () => {
    ...
    expect(response.body).toEqual({ error: "codespace_transfer_not_found" });
  ...
  test("consumes a claimed capability when the client aborts a partial response", async () => {
    ...
    const handler = createCodespaceTransferDownloadHandler(service as never);
    await handler(

...
function copyMigrationsBeforeCodespaceFeature(directory: string): string {
  ...
  const codespaceMigrationIndex = journal.entries.findIndex(
    (entry) => entry.tag === "0027_workspace_connections",
  );
  expect(codespaceMigrationIndex).toBeGreaterThan(0);
  journal.entries = journal.entries.slice(0, codespaceMigrationIndex);
  fs.writeFileSync(journalPath, `${JSON.stringify(journal, null, 2)}\n`);
  return target;
}

describe("Codespace connection schema migration", () => {
  test("upgrades a pre-feature database without changing unrelated data or adding plaintext defaults", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "moira-codespace-migration-"));
    const filename = path.join(directory, "moira.db");
    const preFeatureMigrations = copyMigrationsBeforeCodespaceFeature(directory);
    ...
    try {
      ...
      const codespaceSchema = newTables.map(
        (table) =>
          (
            sqlite
  ...
  test("creates the complete empty codespace schema on a clean database", () => {
  ...
  test("rolls back the codespace schema when its migration fails", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "moira-codespace-rollback-"));
    const preFeatureMigrations = copyMigrationsBeforeCodespaceFeature(directory);
    const failedMigrations = path.join(directory, "failed-migrations");
    fs.cpSync(migrations, failedMigrations, { recursive: true });
    const migrationPath = path.join(failedMigrations, "0027_workspace_connections.sql");
    fs.appendFileSync(
      migrationPath,

...
import {
  ...
  type CodespaceControlAuditEvent,
...
const config: CodespaceGitHubConfigStatus = {
  state: "available",
...
class Provider implements CodespaceProviderAdapter {
...
function fixture() {
  ...
  const controlAudits: CodespaceControlAuditEvent[] = [];
  let currentTime = now;
  ...
  const observability = new CodespaceObservabilityService({
    ...
    operations: new CodespaceOperationRepository(sqlite),
...
describe("codespace kill switches and shared readiness over real SQLite", () => {
  test("a global control refuses new work, stops persistent codespaces, audits and never deletes", async () => {
    const value = fixture();
    try {
      ...
      } satisfies Partial<CodespaceResourceError>);
      expect(value.provider.createCalls).toHaveBeenCalledTimes(1);

      await value.service.reconcileOnce();
      const stored = value.service.getCodespace("user-1", created.resource.id);

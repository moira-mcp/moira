...
function copyBeforeBudgetRemoval(directory: string): string {
  const target = path.join(directory, "before-workspace-daily-budget-removal");
...
describe("workspace daily budget removal migration", () => {
  test("drops submitted-operation accounting while preserving required cleanups", () => {
    const directory = fs.mkdtempSync(path.join(os.tmpdir(), "moira-workspace-budget-migration-"));
    ...
    try {
      ...
      const columns = (
        sqlite.prepare("PRAGMA table_info(workspacePolicyUsage)").all() as Array<{ name: string }>
      ).map(({ name }) => name);
      ...
      expect(sqlite.prepare("SELECT * FROM workspacePolicyUsage").all()).toEqual([
        {
          userId: "user-1",

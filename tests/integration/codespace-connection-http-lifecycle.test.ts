...
describe("GitHub codespace authenticated HTTP lifecycle", () => {
  ...
  beforeEach(() => {
    ...
    const service = new CodespaceConnectionService({
  ...
  test("drives start, callback, status and disconnect without exposing callback or token secrets", async () => {
    ...
    const persisted = JSON.stringify({
      connection: sqlite.prepare("SELECT * FROM codespaceConnection").all(),
      vault: sqlite.prepare("SELECT * FROM codespaceCredentialVault").all(),
    });

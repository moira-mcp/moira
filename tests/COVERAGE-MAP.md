...
- `tests/unit/workflow-engine/execution-progress-lists.test.ts` — bound lists and pass timings of the run projection: an items array with titles, done/total and the current item; counters alone with a zero-based cursor; a finished cursor; an unresolvable binding degrading to no list with a diagnostic; the list following the route cursor; closed passes timed, the open pass measured to the projection moment, per-item durations; runs without timestamps reporting null; a finished run's last pass closed by its exit; the notification line naming done/total and the current item of the bound block nearest the run
- `tests/unit/workflow-engine/execution-statistics.test.ts` — per-version duration statistics: median, quartiles and sample counts per block over the runs stamped with one version, another version and unstamped runs excluded (the latter counted), the asking run left out of its own sample, runs without timestamps contributing nothing; the statistics service caching by the repository's per-version summary and refreshing when a run of the version changes

...
- `tests/e2e/flow-page.spec.ts` — the flow page on Quick Task in its two views: the map by default (seven contents rows, no run status on a definition, typical durations in the panel, a muted cycle edge labelled on chip hover, the contents finder selecting the block that owns a step) and the technical graph with its controls and node sidebar while the block panel steps aside; every link written for a removed view (`outline`, `canvas`, `lanes`, `split`) and any unknown value resolving to the map; a block-panel step switching the page to the graph; the walkthrough anchored on the map's own elements; a non-owner without edit mode; an owner's edit session on a private copy (a block renamed in its panel to a ninety-character name that the map card clamps to two lines with the name box and the facts line inside the card, a return relabelled in the block it re-enters, a moved routing node reported as an `unlabeled-edge` diagnostic that disables the save until moved back, an edited directive, expression, registry default and whole registry declaration as JSON Schema, the inline diagnostic on the moved step, the export listing exactly the five changed flow-file entries, a save that persists, advances the revision to 1 and re-derives on reload); a save refused on a stale revision (409, edits kept, text asks to reload) and on an invalid definition (400, edits kept); and a phone-width page with the contents under the diagram, the panel under both and no horizontal overflow
- `tests/e2e/execution-progress-ui.spec.ts` — the run page on a real Quick Task run with a repair loop, in its two views: on the map the block cards with status, pass count and measured time, the contents sidebar with the same statuses, the repair loop as a cycle edge named by a chip that lights it on hover, and the block panel with the block's three passes, its total, its recorded visits and its steps with expected evidence; a block deep-linked from the card, from the contents and from the step finder, a block the run never entered reading "—" instead of a zero, and a step switching the page to the graph and focusing it (two steps, two viewport transforms); the route cursor moving the whole page back through the run from the panel's visit list; the graph deep-linked, mounting the map only when it is first shown, and giving every output of the decision after the review its own source handle; every link written for a removed view (`lanes`, `canvas`, `outline`, `route`) and any unknown value resolving to the map; the walkthrough in the URL; answering the waiting step from the page (required fields gating submit, a schema-invalid answer refused with the step's message, the accepted answer moving the map and recorded as the person's adjustment in the block's facts, the agent's stale attempt), the answered variable's history opened under its row and its value read at the start visit and without a cursor, the variables table keeping its names inside the panel with long values, the loading state, and a phone-width page kept usable (contents under the diagram, folded mode note, picture height kept, no horizontal scroll) when the projection fails or the workflow has no process view
- `tests/e2e/sdf-progress-ui.spec.ts` — one bounded real SDF path from MCP start through a successfully consumed PNG progress-image token, one answered intake step, and the run page's map: the Intake block waiting on the agent with a measured pass, the Implement block's bound list reported as `done/total` on its contents row and as a checklist in its panel, and the answer form for the step the run then waits on; exhaustive phase semantics remain unit-owned
- `tests/e2e/two-views.spec.ts` — the map and the graph as one page, and the waiting actor of #233: on the run page and the flow page alike a block, a panel tab, a step, a graph node and a view tab change only URL and page state — no page loader, no diagram skeleton, no "laying out" placeholder, the page shell keeping its DOM identity, the map keeping its selected block and the graph its viewport across a switch away and back; the map giving the diagram the column at 1440×900 (the explanation one closed line, the diagram at least 600 px tall, every card at least 60 px tall and the block it opens on wholly inside the diagram’s box) and reading as one scrolling column at 390×844 (the diagram at least 300 px tall, the contents beneath it and the last block reachable by scrolling, no horizontal overflow); a Quick Task paused on its first directive wording the wait as the agent's turn in the card chip, the legend and the block panel and nowhere claiming a person is waited for, while a run seeded at a `lock` node's PIN gate reads as "waiting for you"
...
- `tests/unit/web-frontend/workflow-node-connections.test.tsx` — the node panel's outgoing connections on a four-way routing node: every chip names the output it leaves through (so three edges into one target stay apart), carries the case that selects that output and nothing for an output without a case, and keeps `output → target` in its tooltip
...
- `tests/unit/workflow-engine/execution-progress-image.test.ts` — shared rich visual model, width-metric wrapping that keeps every word and ellipsises a token wider than its line (header and block title), deterministic multi-row and loop/cross-row edges, SVG escaping, semantic content in PNG, byte determinism, theme/state differences, and image metadata bounds; the block card's status word in the map's wording (`agent on the step` for the active block and for a run waiting for the agent, `waiting for you` only for a run waiting for the user, `completed`, `repeated ×n` with the badge, `skipped` struck through, `pending`) in the node `<title>` and drawn text of both views; the facts line with the time spent and the open pass, `done/total: current item` for a bound block and no count for an unbound one, `—` never `0 s`, ellipsised on one line at the card's text width; the duration wording (`12 s`, `1 min 20 s`, `2 h 05 min`, `—`); every derived transition into a hub is a drawn connector in the process view, sources into one hub sharing one bundled lane and one port, labelled inside the source
- `tests/unit/workflow-engine/execution-progress-geometry.test.ts` — the progress image keeps every text inside its box and every label inside the canvas: on the six annotated flows (blocks repeated twelve times) and the bundled SDF run projection (long titles, a bound list with a long current item, a repeated block, timings) at 480, 720 and 1280 px, in both views and both themes, every node line (title, status word, facts, content with its marker), fact-chip line and header line measured with the model's metric at the model's type scale fits the width it was wrapped for, every node, fact and label box lies inside the image, a phone width lays out one column with the phone type scale (title ≥ 18 px, content ≥ 14 px, labels ≥ 12 px) and content never drops below 12 px, every label box is disjoint from other labels, blocks and other arcs' lane runs, every repeat badge sits between the mark and the title inside its block, every label carries its whole text, the model is deterministic; the text metric orders glyph widths and weights, width wrapping keeps every word and ellipsises a word wider than the line
...
- `tests/unit/web-frontend/run-layout.test.ts` — canvas layout of the six annotated bundled flows through ELK: no overlapping blocks, every transition drawn as an edge (a hub target as a bundled hub edge into one port plus a chip in the source, never crossing a block), forward edges never pointing backwards, deterministic placement (rows included, on a multi-branch flow too), hubs after their sources, and parallel forward transitions between one pair of adjacent blocks on distinct paths and label rows, and every forward label pill drawn at rest inside its gap clear of both blocks (the gap grows to the widest pill); the rows of the map, block by block, over the Software Development Flow, Robust Task, Test Generation, Smart Purchase Assistant, Test Planning and PRD Creation — the main sequence (its hubs included, `deliver` rather than `repair` where the chains tie) on one row with equal top edges, each side branch on a row of its own, branches whose column spans overlap on separate rows and disjoint ones sharing a row, an off-sequence hub and a loop-only block each on a row of its own beneath — plus three synthetic processes (overlapping branches on opposite sides of the sequence, disjoint branches sharing a row, a block entered only by a return beneath); no skip, return or hub-bundle segment passing through a block on the stacked rows; the block height estimate charging a name no more than two lines, the clamp the card applies
- `tests/unit/web-frontend/run-map-view.test.tsx` — the map view over a bundled flow's projection: the header band names the task, the title the run rendered, the goal and the projection's fact chips with their tones; the contents list every block in process order with its position, name, status, pass count and the bound list's done/total, mark the selected block and the block the run is on, and carry the node finder; the diagram's cards show the pass count, the time spent in the block, the open pass's own time and the bound list's done/total, and a block the run has not measured shows no time at all (never "0 s"); a card clamps a ninety-character name to two lines and keeps the whole name in its `title`; the view's explanation is one collapsed row whose body is absent until it is opened and whose open state is remembered in `localStorage` per page; the diagram's box clips its content, mounts no minimap, and on a phone is a fixed-height column with the contents after it
- `tests/unit/web-frontend/run-view-modes.test.ts` — the two view modes of both pages: each registry holds exactly `map` then `graph` with distinct icons and the map as the default; every retired id (`lanes`, `canvas`, `outline`, `route`, `split`) and any unknown, empty, null or undefined value resolves to the map; the run page's walkthrough names no unregistered view, every step is anchored from whichever view is open (its own selector or its fallback's), and the map anchors the process, loop and route steps
- `tests/unit/web-frontend/run-transition-chips.test.ts` — the chip model of the map's diagram over the six annotated flows: the chip keys equal the keys of the laid-out cycle, skip and hub edges, each side unique; a source with several transitions into one hub gets one chip carrying every label; three or more adjacent forward transitions into one block fold into one forward chip (fewer get none) and the edge keys include such bundles; the SDF lists all of its returns; a chip's title carries the label and a return's cause and exit
...
- `tests/unit/config/workspace-github-deployment-config.test.ts` — complete distinct GitHub App/vault environment names in examples and operator-owned Compose references with no committed secret value, and a Compose fallback policy identical to the code defaults
- `tests/unit/web-backend/workspace-connection-routes.test.ts` — browser redirect, secret-free successful/failed/early-status callback outcomes, post-installation GitHub return without state restarting authorization, generic sensitive-query suppression, explicit external-revocation confirmation and sanitized status projection at the HTTP boundary
- `tests/unit/web-backend/github-workspace-client.test.ts` — authorization-code and refresh exchanges for expiring user tokens, refresh-error secrecy, bounded same-origin pagination, lossless GitHub IDs and app-owned revocation request boundaries; the refusal reason read from a provider's body: a bare message, field errors and an HTML body all carried, an empty body and every secret-, token-, JWT- or URL-shaped message dropped rather than partially redacted, a long message bounded, and a refusing request throwing with the reason attached
...
- `tests/unit/shared/workspace-resource-policy.test.ts` — disabled safe defaults, finite lifecycle/operation/transfer ceilings, absence of any per-day operation budget including an ignored removed variable, unit conversion, shipped multi-workspace active ceilings with the instance ceiling above the per-user one, a retained-output ceiling far above the response payload bound that fails closed when configured below it, a background-command ceiling above the bounded-command ceiling, a bounded wait for a workspace to start with its unit conversion and both ends refused, and fail-closed malformed or inconsistent resource and aggregate transfer limits
- `tests/unit/web-backend/github-codespaces-provider.test.ts` — exact REST methods/paths, explicit machine/marker/retention, a definitive rejection carrying the provider's constraint sentence while a rejection whose message echoes a credential carries none, machine and bounded same-origin Codespace pagination, immutable owner/billable/repository/machine parsing, unknown-state containment, background acceptance, sanitized definitive/unknown failures and exact absence
...
- `tests/unit/web-backend/workspace-management-routes.test.ts` — website workspace list/create/get/start/stop/delete over the shared services: no-store headers, sanitized summaries, fail-closed not-configured with same-origin Settings link, create without lifecycle capability, malformed input rejection, confirmed generation-fenced delete, bounded status mapping that withholds the operator sentence while showing the bounded refusal detail, and metadata-only operation history
...
- `tests/unit/mcp-server/manage-workspaces.test.ts` — opaque-ID-only pre-validation log context, secret-free workspace/operation projections against sentinel internal fields, private creation capability, stop/delete distinction with confirmed generation, exclusive text/native stdin dispatch without reference echo, resume by operation ID without re-dispatch, terminal exec/file failures as tool errors retaining operation identity, UTF-8 read/patch/upload/download projection without base64, native `resource_link` without raw URL, bounded setup/foreign-workspace/binary-read errors, a policy-limit refusal that names its ceiling while an undetailed refusal stays generic, session parameters passed to the service with nothing a session stores echoed back, a truncated exec payload reported with its complete retained size, a retained range read that dispatches no file operation and is refused when the named workspace does not own the command, a command stopped by the retained ceiling reported as that, a command lost to a workspace restart projected as interrupted with its own error code rather than as an ordinary failure, and server-side reporting of unexpected failures behind the generic internal error; the provider's own reason for a refused creation reaching the caller, non-retryable; one call per action through the single `workspace` tool, a per-action row refusing a field that belongs to another action with the identity `WORKSPACE_REQUEST_INVALID`, the mirror row per action parsing that action's boundary payload through the published object and dispatch so a bound lost while merging a shared field name fails here, an unknown action and a missing action refused before any contract is chosen, a field belonging to no action refused by the published object itself, and the action enum covering every dispatched action

**integration**

- `tests/integration/workspace-connections-migration.test.ts` — migration 0027 clean install, representative existing-database preservation, transactional failure rollback and ciphertext-only credential/revocation storage
...
- `tests/integration/workspace-resources-migration.test.ts` — migration 0028 preserves Unit 1 connection data, creates secret-free lifecycle authority atomically and rolls back injected failure
- `tests/integration/workspace-resource-lifecycle.test.ts` — persistent create/list/get/start/stop/delete, a caller-facing listing that omits deleted and rejected workspaces while they stay addressable by identifier, typed mapping of provider HTTP 401/403/404/5xx before reservation, pre-submission disconnect, submitted create/stop and start/stop races, explicit generation-fenced deletion, provider-absence convergence, same-account/grant reauthorization, tenant isolation, durable lifecycle intent/accounting, exact identity, response-loss reconciliation, legacy cleanup, several concurrent workspaces per user under the shipped ceilings, resource limits distinguished as per-user ceiling, instance ceiling or create throttle, a stopped workspace woken for the work that needs it with one start and none for a workspace already awake, a deleted one refused by what it is, a started workspace that never becomes usable refused by the wait it exhausted with its provider traffic bounded by that wait, and kill switches; the provider's own reason for a refused creation reaching the record and the audit entry, a refused start classified per status with the reason kept and a permanent refusal staying non-retryable, a refused start, stop or delete audited under the operation it refused with that reason, and a failure that is not a provider refusal staying an internal failure and staying out of the audit trail
- `tests/integration/workspace-operation-lifecycle.test.ts` — provider-neutral operation reservation, canonical workspace projection, argv/stdin/result non-persistence, unbounded same-day submission, terminal results returned by a bounded post-dispatch settle window with a single dispatch and one terminal event, a truncated payload reported with its complete retained size, fenced range reads of retained output that report expiry once cleanup removed it and refuse an unavailable connector before contacting it, a background command answered immediately without a settle window and collected later through one dispatch, the duration and result-retention window each mode implies, a lifetime granted at dispatch rather than at reservation, a command whose session is gone refused without running and without holding capacity, both session ceilings refused as bounded policy outcomes, one command form per call, plus malformed session names and variables refused before reservation, a sleeping workspace started by the command that needs it while the same workspace without lifecycle authority is still refused before credentials, a start refusal that holds no capacity, a command lost to a workspace restart completed with its own recorded reason, each command kind bounded by its own ceiling with a refusal naming it, and cancellation of a background command, concurrent limits, cross-tenant and desired-state fencing, pre-dispatch credential failure, crash-abandoned reservation expiry without connector contact, durable dispatch intent and post-crash exact-marker recovery after claim expiry, unknown remote outcomes, background-observed result retrieval before bounded cleanup, cancellation/generation races and retryable remote finalization; one-call structured native-reference fetch/store/exact-binary dispatch, tenant-bound internal reference stdin, size/MIME/unavailable rejection, and foreign/missing/stopped/disabled/busy workspace rejection before source fetch, credentials or provider contact, plus claim release and consumption after durable dispatch
...
- `tests/integration/workspace-daily-budget-removal-migration.test.ts` — upgrade of a populated policy-usage table: dropped submitted-operation accounting, preserved required cleanups and a still-enforced user/provider/day key
...
- `tests/integration/workspace-mcp-client-flow.test.ts` — ChatGPT-compatible MCP client over the real registry, request context, connection/vault, SQLite, resource/operation/file/transfer services, production connector framing and remote supervisor: actual file write/stat/read/search/patch, executed passing/failing commands with exact exits, Git diff, native upload, exact native-stdin digest, text stdin, consumed download bytes, workspace reuse after reopened SQLite and a fresh client, stop preserving files, generation-fenced delete after which discovery omits the workspace while a direct lookup still answers; a background command that returns running immediately, is watched through retained-output ranges while it runs, is collected later through one dispatch and is stopped on request; a session whose working directory and variables reach later commands while a command outside it observes neither, a script that activates a toolchain followed by an ordinary command that inherits it, ending a session and being refused afterwards, with nothing stored or echoed and an unopened session refused; lost write/exec/download recovery by operation ID with one remote dispatch, foreign-user refusal and content/credential-free operation rows

**mcp-tools**

- `tests/mcp-tools/workspace-tools.test.ts` — published single `workspace` tool whose action enum offers every action it dispatches, instance readiness from the `list` action matching MCP `/health` with top-level native file parameters and resume schemas, safe default-disabled readiness with same-origin Settings link, private download routing and exact single-use byte delivery with protective headers through the public origin, website-only setup errors over HTTP, the published exec catalog carrying the session form an agent needs to ask for one, and boundary rejection of mixed stdin or agent-supplied auth/chat/session fields
...
- `tests/unit/workflow-engine/execution-progress-geometry.test.ts` — every text line of the progress image fits its box and every label lies inside the canvas on the six flows and the SDF run at three widths, both views and themes; label and badge boxes never overlap; metric, width wrapping and ellipsis
...
- `tests/unit/web-frontend/run-layout.test.ts` — deterministic non-overlapping ELK canvas layout of every annotated bundled flow, with the process rows (sequence, branches, hubs and loop-only blocks beneath) and no edge through a block
- `tests/unit/web-frontend/run-map-view.test.tsx` — the map's header band, contents list, card run facts (passes, times, bound list), collapsed one-row guidance and phone column
- `tests/unit/web-frontend/run-view-modes.test.ts` — two view modes per page, retired ids resolving to the map, walkthrough anchors
- `tests/unit/web-frontend/run-transition-chips.test.ts` — the map's chip model: chip keys equal the diagram's connector keys; hub bundles; titles

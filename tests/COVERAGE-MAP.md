...
- `tests/mcp-tools/workspace-tools.test.ts` — published fourteen-tool catalog with top-level native file parameters and resume schemas, safe default-disabled readiness with same-origin Settings link, private download routing and exact single-use byte delivery with protective headers through the public origin, website-only setup errors over HTTP, and boundary rejection of mixed stdin or agent-supplied auth/chat/session fields

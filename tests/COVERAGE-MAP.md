...
- `tests/unit/workflow-cli/workflow-derive.test.ts` — `derive` output of the annotated Quick Task (blocks, labelled transitions, explained returns, no diagnostics), diagnostics of a not-yet-annotated flow, the no-progress message, and determinism

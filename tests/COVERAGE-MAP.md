...
- `tests/unit/shared/workspace-resource-policy.test.ts` — disabled safe defaults, finite lifecycle and operation ceilings, unit conversion and fail-closed malformed/inconsistent resource limits
...
- `tests/unit/web-backend/github-codespaces-connector.test.ts` — Unix-socket credential delivery, bounded wire envelopes at the accepted raw input/output maxima, adversarial argv and exact exit transport, explicit remote finalization, token-free options and secret-free generated SSH capability probe
...
- `tests/unit/web-backend/github-codespaces-remote-supervisor.test.ts` — direct Codespace-user argv/stdin execution, exact exit and separated output, repeatable terminal-result inspection, atomic runner publication after child exit, timeout/output bounds, PID start-time fencing and verified process-group cancellation
...
- `tests/integration/workspace-operation-lifecycle.test.ts` — provider-neutral operation reservation, canonical workspace projection, argv/stdin/result non-persistence, daily/concurrent limits, cross-tenant and desired-state fencing, pre-dispatch credential failure, crash-abandoned reservation expiry without connector contact, durable dispatch intent and post-crash exact-marker recovery after claim expiry, unknown remote outcomes, background-observed result retrieval before bounded cleanup, cancellation/generation races and retryable remote finalization

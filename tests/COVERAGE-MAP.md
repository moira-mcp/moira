...
- `tests/integration/marketplace-gallery-partition.test.ts` — 3 tests 🟢 (Step 9 storefront partition facets against a migrated DB: getGalleryFacets returns all = official + community (official=system-moira owned, community=everyone else); the `community` gallery filter returns ONLY non-official listings and is disjoint from the `official` filter (official.total + community.total = all.total); facet counts respect an active search)
...
- `tests/integration/marketplace-slug-and-self-install.test.ts` — 6 tests 🟢 (Step 8 data-hygiene fixes against a migrated DB. D-N4 slug-freed-on-delete: soft-deleting a flow frees its slug (the row's slug is moved to `deleted-<id>`) so re-creating a flow with the SAME slug succeeds (two distinct live/dead rows), and `slugExists` excludes deleted rows; `restore()` after the freed slug was re-taken assigns a fresh non-colliding slug while the replacement keeps the slug; **legacy-data repair (D1): a row soft-deleted before the enforcement still holds its real slug → re-create fails at the DB unique index, then `repairFreedSlugs()` frees it (=1), reuse succeeds, and a second run is idempotent (=0) — the canonical typed equivalent of the scripts/run-migrations.ts startup repair**. D-N5 self-install guard: the listing author installing their own listing is rejected with SelfInstallError and `installCount` stays 0; a non-author install counts (=1) but a subsequent author self-install does not (stays 1); the author forking their own listing is allowed (independent copy) but does NOT inflate the counter (stays 0) while a non-author fork increments it (=1))
...
- `tests/e2e/marketplace-storefront-partition.spec.ts` — 4 tests 🟢 (Playwright, Step 9 SSR storefront UX, anonymous view: a no-match `?search=` shows the search-specific empty state (`empty-search` + "No workflows match your filters." + a `clear-search` link back to the non-empty catalog) and the count reads "0 results" (NOT "No published workflows yet." / "N published") — D-N11; three partition chips All/Official/Community each carry a `chip-count` and Official+Community = All with Community ≥ 1 — D-N8; clicking the Community chip navigates to `?community=true` (aria-pressed) and shows the community flow; at a 360px viewport the card list has no horizontal overflow and the first card title is fully within the viewport — D-N10)
...
- `tests/unit/marketplace-render/render-html.test.ts` — 31 tests 🟢 (Step 9 additions: ExploreGallery empty-state — a truly empty catalog shows `data-mp="empty"` + "No published workflows yet." while a no-results search/filter shows `data-mp="empty-search"` + "No workflows match your filters." + a `clear-search` link, and the count switches from "N published" to "N results" (EN + RU "результат" declensions) — D-N11; partition-aware chips — three crawlable chips All/Official/Community each carrying their facet count (sums to a partition), Community chip `?community=true` with active `aria-pressed`, chip links preserve the active `?search`, counts omitted when no facets — D-N8. PLUS the prior @mcp-moira/marketplace-render server render coverage: renderExploreToHtml crawlable semantic gallery HTML + real detail `<a href>` links + escaped SEO head (title/description/canonical/OG/Twitter) + well-formed ItemList+BreadcrumbList JSON-LD; renderDetailToHtml semantic `<article>` + start command + SoftwareApplication JSON-LD with/without aggregateRating + BreadcrumbList; localized counts EN vs RU CLDR forms (published/steps "2 шага"/installs "5 установок"/"1 install"); localized category enum (RU "Исследования"/"Данные и анализ", not raw) + "Unrated"/"Без оценок"; viewer pills only when authenticated+annotated; JSON-LD XSS-safety — malicious detail/gallery title+summary cannot break out of `<script type="application/ld+json">` (`<`-escaped, no raw `</script>`/`<script>alert`/`<img onerror`); session-aware detail action area (Step 21) — anonymous → sign-in CTA + public Download link + NO adopt-btn, signed-in addable viewer → `adopt-btn` "Add to library", owner / in-library viewer → NO adopt-btn but Download still present; ExploreGallery Official filter chips — All/Official crawlable LINK chips carry `?official`/`?lang` with the active `aria-pressed` driven by the threaded filter, localized chip labels (Все/Официальные) + `&`-escaped combined query; ListingCard adopt+download — addable card shows adopt-btn + download-link, in-library/anonymous cards show download-link only)

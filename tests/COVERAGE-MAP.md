...
- `tests/integration/workspace-operation-lifecycle.test.ts` — provider-neutral operation reservation, canonical workspace projection, argv/stdin/result non-persistence, unbounded same-day submission, concurrent limits, cross-tenant and desired-state fencing, pre-dispatch credential failure, crash-abandoned reservation expiry without connector contact, durable dispatch intent and post-crash exact-marker recovery after claim expiry, unknown remote outcomes, background-observed result retrieval before bounded cleanup, cancellation/generation races and retryable remote finalization; one-call structured native-reference fetch/store/exact-binary dispatch, tenant-bound internal reference stdin, size/MIME/unavailable rejection, and foreign/missing/stopped/disabled/busy workspace rejection before source fetch, credentials or provider contact, plus claim release and consumption after durable dispatch

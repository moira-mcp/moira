...
- `tests/unit/workflow-engine/execution-progress-lists.test.ts` — bound lists and pass timings of the run projection: an items array with titles, done/total and the current item; counters alone with a zero-based cursor; a finished cursor; an unresolvable binding degrading to no list with a diagnostic; the list following the route cursor; closed passes timed, the open pass measured to the projection moment and growing between two projections, per-item durations, a pass before the cursor's first write attributed by the registry default or to no item, an unresolvable title path keeping the list with a diagnostic; runs without timestamps reporting null; a finished run's last pass closed by its exit; the notification line naming done/total and the current item of the bound block nearest the run

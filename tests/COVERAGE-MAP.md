...
- `tests/unit/scripts/git-tree-identity.test.ts` — the image build identity: a clean checkout is named by its commit, and an uncommitted binary change larger than a pipe buffer still yields `<commit>-dirty-<hash>` (not the build script's silent `unknown`), changing with the content
...
- `tests/e2e/onboarding.spec.ts` — a newcomer's path: the home page's three agent-first steps (connect, describe, the agent picks or builds the flow through the Workflow Management Flow), the connection card and the compact recommended flows in English and Russian; the flow list's recommended section (learning examples in the reader's language, then Quick Task, Robust Task and Todo List with when to pick them), its fold remembered across a reload, the list's filters folded behind "Filters" and counted while in effect; a learning example opening on the steps view as numbered instruction cards joined by edges with only the fork's answers labelled, in English and in Russian, and the map one click away; and "variables as words" on Todo List turning every `{{…}}` into words, off in the link as `inline=0`
- `tests/e2e/beginner-panels.spec.ts` — each beginner panel (home intro, connect your agent, home and flow-list recommended flows) hides from its own control and stays hidden after a reload and in a new browser for the same user; the notice's undo restores it; Settings → Preferences lists the six panels and shows a hidden one again; the flow page's variables hint hides for the account from its close button; on the flow list the fold is per browser while the hide is per account
...
- `tests/e2e/docs-serving.spec.ts` — built Starlight docs served at /docs in the image: /docs serves Starlight not the Web UI SPA; /ru/docs RU; / still Web UI; missing doc 404s instead of SPA fallthrough; the light and dark themes each paint their own page background (the site palette does not override Starlight's light theme)
...
- `tests/e2e/admin-tokens.spec.ts` — admin token list with user info, name search (exactly one match), status filter and revoke; the user and token are created with fresh names on every run of the setup, which a worker repeats when it returns to the file

### artifacts

**unit**

- `tests/unit/shared/artifact-service.test.ts` — artifact validation and service rules; long unterminated tag-like content rejected in bounded process CPU time
...
- `tests/e2e/audit-log.spec.ts` — audit log viewing, filtering and details, and an entry whose user can no longer be found named in Russian rather than with an English placeholder
...
- `tests/unit/mcp-server/messages.test.ts` — attempt-error categorization, including identical processing retries, automatic `current_step` recovery for stale, conflicting, and explicitly recoverable unavailable step attempts, rejected-input disposal, retained no-retry boundaries for unavailable starts and outcome-unknown, including the post-handler `… retry from current state` conflicts; stale-state conflicts (every `… reload …`, `… is stale`, workflow revision conflict) recover by re-reading and retrying; only human-boundary categories (outcome-unknown, workflow not found, auth, connection, access) say stop, and an unrecognised error tells the agent to diagnose and fix, asking the user only for what it cannot obtain
...
- `tests/unit/web-frontend/account-approval-admin-ui.test.tsx` — capability-aware account-approval status and actions in the administrator list/detail surfaces (on the list card a pending approval is a badge and an approved account a quiet fact on the meta line, both the approval focus target), including SaaS null-timestamp behavior, self-host suppression of the artifact-quota request/card, temporary-password form submission, real versus unavailable profile/admin delivery controls, and fail-closed forgot-password loading/error/unavailable states
...
- `tests/mcp-tools/beginner-examples.test.ts` — the learning examples run end to end through `start` and `step` against the bundled catalog: Example 1's four steps in English, Example 2's "no" branch in Russian with the Russian instructions shown, Example 3's bug path
...
- `tests/unit/workflow-engine/extension-runner-client-http.test.ts` — transport deadlines, metadata status/envelope/version authority, and mapping of HTTP/network responses to extension failure classes; the metadata deadline checked on a fake clock (pending at 49 ms, failed at 50 ms)
...
- `tests/unit/web-frontend/destructive-contrast.test.ts` — WCAG AA (4.5:1) for destructive text on the page, card and popover surfaces and for every destructive fill pair (foreground and white on the fill), in both themes, computed from the `globals.css` tokens with an OKLCH→sRGB→WCAG function checked against reference ratios; every component drawing text on a solid destructive fill uses the fill token
- `tests/unit/web-frontend/codespace-error-message.test.ts` — a codespace failure worded from its code in the reader's language (known code, unknown code, no code), every code with EN and RU messages
- `tests/unit/web-frontend/graph-routing-performance.test.ts` — the 300-card fan-in layout within its budget in each preset, measured as the benchmark process's CPU time
...
- `tests/api/settings-api.test.ts` — authenticated settings CRUD plus explicit mixed bulk 207 results, unknown-key continuation and admin-only definition/value visibility rules on the real container; the per-key routes exercised on a test-owned definition; the retired `ui.theme` no longer offered; the hidden beginner panels are a seeded non-admin `ui` json setting that starts empty and stores a bulk-saved list per user; every write path (bulk and per-key) refuses a value outside its declared schema (a bare string, a repeated id, non-strings) while the user's settings stay readable, and the per-key route stores a valid list
- `tests/api/admin-missing-user-email.test.ts` — admin audit and email logs for a user who can no longer be found report the email as `null`, never the literal "Unknown"
...
- `tests/unit/web-frontend/run-view-modes.test.ts` — the view modes of both pages: the run page holds exactly `map` then `graph`, the flow page `steps`, `map`, `graph`, each with distinct icons and the map as the default; every retired id (`lanes`, `canvas`, `outline`, `route`, `split`) and any unknown, empty, null or undefined value resolves to the page's default, which the flow page may set to the steps view; both walkthroughs name no unregistered view and reach every step from every view, the run page's anchoring every step in the graph as well as the map; the flow page's opening on the agent-first header step and the steps step (the instruction cards on the steps view, the Steps switch on the map and the graph), and the anchors are the redesigned elements: a contents row, a card's return port, the route cursor, each view's own toolbar, and the editing step pointing at the edit toggle for an owner and at the header for a reader
- `tests/unit/web-frontend/steps-list.test.tsx` — the steps view's reading list ends every card with its way on: "Next:" with a link for a single way, each labelled choice linked to its step, "back to step N" marked as a return; every card is an anchor target; a link to a check names its place ("…, before step 1") and a return to it reads "back to: …"; step labels are `h2` headings named "Step N" (the number badge hidden from assistive technology) and every card can take focus; two rows that would read alike get their order ("(#2)"), so every link names one place
- `tests/e2e/steps-list.spec.ts` — the Software Development Flow in the steps view reads as a list: no canvas or zoom, step 1 in view at normal size, a fork's labelled choice followed from the keyboard brings its step into view and moves focus there, a `#step-<id>` link opens the list at that step on a fresh load and when set on the open page, then leaves the address, a return names the step it goes back to, variables as words on by default with no braces (template blocks included) and `inline=0` in the link, and the walkthrough's steps anchor on an instruction card
- `tests/unit/web-frontend/steps-view-model.test.ts` — the steps view's model on the shipped learning examples: numbered instructions joined by unlabelled arrows on a straight sequence, the fork's authored answer labels in English and in Russian, one finish per path, a loop as a single return to the open step with unreachable teleports left out, and nothing to read without a start node; `humanizeVariable` reading variables as words; the recommendation registry offering the examples in the interface language before the universal flows and opening only the system's examples on the steps view; the presentation choice on the catalog: the six learning examples and Todo List drawn (Todo List despite its return), Quick Task, Robust Task and the Software Development Flow as a list
- `tests/unit/web-frontend/recommended-flows.test.tsx` — the recommended section asks the catalog once for the reader's-language slugs and offers only the system-owned flows the answer contains (a same-slug flow of another owner is not offered), and draws nothing when none exists
- `tests/unit/web-frontend/beginner-panels.test.tsx` — the hidden-panel store keeps only known panel ids from the stored list, applies a hide to the stored list so another device's hide is kept, stores every one of several hides made in quick succession, and takes back a hide the server refuses
- `tests/unit/web-frontend/workflow-card-status.test.tsx` — a never-validated flow's badge reads "Not checked" / «Не проверен», like the status filter, and never passes for invalid; a valid flow has no status badge, an invalid one has its badge
- `tests/unit/web-frontend/list-latest-request.test.ts` — every list page (each page sizing a `DataListView` with `useListPageSize`) and the flow list's `useWorkflowList` take the shared `useLatestRequest` hook, drop an older request's answer on success and on error, clear loading only for the latest request, and keep no request counter of their own
- `tests/unit/web-frontend/work-area.test.tsx` — the home work area drawn from a summary: a run in progress carries a badge only for a lock or refusals, a step without a name reads as words from its id, an empty section says so in one line while the others list items, a universal flow of the system owner offers its authored prompt and another flow the template from its name, and nothing at all gives the one empty state
- `tests/unit/web-frontend/work-area-texts.test.ts` — relative times in English and Russian, the authored-prompt lookup only for the system owner's universal flows, and interpolated names read as written (no HTML entities)
- `tests/unit/web-frontend/status-badge.test.tsx` — an execution's status badge reads its status in the interface language (English and Russian) and keeps the raw status as `data-status`
...
- `tests/unit/web-frontend/diagram-placement.test.tsx` — the shared opening placement hook: places once the substrate is ready, again only when the followed key changes, never on a same-key re-render, and waits for ready when the key changes early; reports `placed` only once the placement's camera move has arrived (or a later reader's move has ended), and not for a key a newer placement superseded
- `tests/unit/web-frontend/diagram-measured-nodes.test.tsx` — `useMeasuredNodes` carries a node's last measured size onto the node objects a diagram rebuilds, so a rebuild never leaves React Flow waiting for a measurement; a node that declares its own `measured` keeps it, and a removed node's size is forgotten
...
- `tests/e2e/workflow-toolbar-redesign.spec.ts` — the flow list's toolbar: search in view, status, visibility, sort and reset folded behind "Filters" until opened, pagination indicator, view-mode toggle, pagination reset on a filter change
...
- `tests/unit/shared/logging/sanitize-input.test.ts` — generic secret removal, communication content/filename suppression, masking and bounded diagnostic projection; adversarial email-like input sanitized in bounded process CPU time
...
- `tests/e2e/settings-page.spec.ts` — flat settings layout plus Telegram settings grouped under its metadata-driven communication card with masked secret and persisted enable control, a profile name change confirmed by its toast, the Telegram tour button and the Telegram documentation link named apart, the link named and pointing at the documentation in the reader's language (EN and RU), and the sessions list's heading and help inside its own card; the Telegram enable toggle on a user of its own, read from the channel's state badge
- `tests/e2e/settings-security.spec.ts` — the Security section by sign-in method: a password account changes its password (a wrong current password explained in words), a GitHub-only account is told how it signs in, is offered no current-password field, sets a password and then gets the change form
...
- `tests/unit/docs/learning-examples-docs.test.ts` — the learning flows quoted word for word: every directive and fork answer label of the six flows appears in the tutorial and in the reference page of its language, and the quick start runs the example of its language
...
- `tests/unit/shared/slug-handle-validation.test.ts` — slug and handle rules; long punctuation runs normalized in bounded process CPU time (immune to machine load)
...
- `tests/e2e/dashboard.spec.ts` — the home work area: a new user gets one empty state with the try prompt and no stat card, pointing to the recommended flows while they are shown and to the flow list once that panel is hidden; a returning user's run in progress shows its flow and named step, copies the prompt that asks the agent to continue it, and opens the run; its flow offers its prompt and run count; no dead-end actions; the "Connect your agent" card with its per-client tabs, and the sidebar and connection-card documentation links, which open the Russian documentation for a Russian reader
- `tests/e2e/mobile-navigation.spec.ts`
- `tests/e2e/sidebar.spec.ts`
- `tests/e2e/theme-integration.spec.ts`
- `tests/e2e/theme-loading-state.spec.ts`
- `tests/e2e/verify-step24.spec.ts` — navigation, execution-card behavior, and beta-banner placement and dismissal
- `tests/e2e/visual-regression.spec.ts` — light/dark screenshots of the principal application pages: the theme set through the key the app reads (`theme`), Settings captured through everything `main` scrolls, data pages at a fixed height, volatile regions masked at a fixed size, and a 100-pixel budget that a one-step heading size change on Settings exceeds (runs only with `PLAYWRIGHT_INCLUDE_EXTERNAL=true`; regenerate with `PLAYWRIGHT_INCLUDE_EXTERNAL=true npm run test:e2e -- --file tests/e2e/visual-regression.spec.ts -- --update-snapshots=all`, adding `--grep "<page> - "` for one page)

### codespace-connections

**unit**

- `tests/unit/shared/codespace-github-config.test.ts` — disabled-by-default, complete strong configuration, malformed client/key-version rejection, exact same-origin HTTPS callback, GitHub installation URL and dedicated vault-key validation; app-prefix normalization bounded by process CPU time, not wall-clock time
...
- `tests/unit/config/codespace-github-deployment-config.test.ts` — complete distinct GitHub App/vault environment names in examples and operator-owned Compose references with no committed secret value, and a Compose fallback policy identical to the code defaults; the removed remote expiry offered in neither example env file nor Compose
...
- `tests/unit/web-backend/codespace-connection-routes.test.ts` — browser redirect, secret-free successful/failed/early-status callback outcomes, an App installation return with a stored credential refreshing the grants and landing on Settings as connected or installation_required without exchanging its code, one without a usable credential starting authorization on this site, an authorization that finds no installation going straight to the GitHub install page, a refused start returning the browser to Settings with an outcome (already connected, not configured, previous access not revoked) instead of JSON, generic sensitive-query suppression, explicit external-revocation confirmation, TTL status refresh, forced refresh and stale-snapshot projection at the HTTP boundary; a callback whose connection status cannot be read still redirecting the browser (303) to the Settings path on this site, under the app prefix or at the root, never answering with JSON
...
- `tests/unit/shared/codespace-observability.test.ts` — one shared readiness decision (disabled/misconfigured/control_disabled/connector_unavailable/ready), the health-degradation rule for every state, no connector probe while disabled, bounded probe timeout with cached liveness snapshot reuse and one shared recomputation, identifier-free readiness projection, gauge values, closed-label connection/lifecycle/operation/rejection metrics and unknown-code rejection; readiness reporting the enforced operation and live-transfer ceilings from the one limits source when the policy leaves them unset
- `tests/unit/web-backend/codespace-management-routes.test.ts` — website codespace list (with the caller's own limits view)/create/get/start/stop/delete over the shared services: no-store headers, sanitized summaries, fail-closed not-configured with same-origin Settings link, create without lifecycle capability, malformed input rejection, confirmed generation-fenced delete, bounded status mapping that withholds the operator sentence while showing the bounded refusal detail, and metadata-only operation history
...
- `tests/integration/codespace-connection-service.test.ts` — no forced account chooser, an installation return connecting with the stored credential and no second exchange or revocation (and keeping an existing connection; asking for authorization without a usable credential), a Settings read inside the grants cache showing a new installation while installation_required, a reauthorization whose old-token revocation fails still committing the new credential (generation +1, connected, codespaces rebound) with the superseded credential queued in the same commit as its successor (still queued when nothing after the commit runs) and revoked by the next grant refresh or disconnect, a successor that fails validation leaving the working credential unrevoked, expired/session/cross-tenant one-time state, foreign-installation rejection before repository enumeration, GitHub identity/install/repository binding, encrypted cross-tenant persistence, ten-minute repository/install grant refresh with forced refresh, same-process refresh single-flight, failed-attempt rate limiting, stale fallback without destructive replacement, malformed-name containment and installation-required/connected transitions, plus credential-refresh single-flight, persisted pre-provider recovery authority and successor disposition across staging/provider/CAS/disconnect failures, submitted-lease waiter recovery, ambiguous/expired/abandoned refresh failure, classic-OAuth rejection, combined exact pending revocation, active and pending-only unreadable recovery, restored-key exact disconnect, and installation-required behavior; a stateless installation return inside the grant-refresh failure window making no provider call while a normal return refreshes exactly once; a database error rolling back the credential commit leaving the previous credential usable and the connection as it was
- `tests/integration/codespace-connection-http-lifecycle.test.ts` — real authenticated route, connection service, vault and SQLite start/callback/status/disconnect lifecycle with only outbound GitHub transport substituted and callback/token secrecy observed; one Connect with one GitHub authorization continuing straight to the App install page and returning to connected with no second exchange and no revocation
...
- `tests/unit/shared/workflow-query-service.test.ts` — incl. setWorkflowVariable preserves rich schema; regex-safety of variable analysis and search bounded by process CPU time
...
- `tests/unit/web-frontend/run-view-modes.test.ts` — the view modes per page (the flow page's steps view first), retired ids resolving to the page's default, both pages' walkthrough anchors reachable from every view
- `tests/unit/web-frontend/steps-view-model.test.ts` — the steps view's model on the learning examples and a looping flow, variables as words, the recommended flows per language
...
- `tests/unit/web-frontend/diagram-primitives.test.tsx` — variable tokens with their braces, template blocks read as words, the one tooltip's surface and bodies, the foldable panel section
...
- `tests/api/dashboard-summary.test.ts` — `GET /api/stats/summary`: a new user gets three empty lists; a run in progress carries the step it waits at, named by its active progress label, else its display name, else its progress block; a finished run is only in the recently finished list; the flows run most come first with exact counts, ties broken by the latest run, and a deleted flow is left out
- `tests/api/workflow-list-access.test.ts` — `GET /api/workflows?access=`: `mine` is exactly the reader's flows with two name-sorted pages continuing each other, `shared` is what a grant gives and not what is owned, `catalog` is others' public flows paged without overlap, the scope combines with visibility and search, and the status filter selects exactly a real invalid flow (`invalid`), exactly the valid ones (`valid`) and none (`unknown`) among the reader's flows, on the page and in the total
- `tests/api/workflow-list-slugs.test.ts` — `GET /api/workflows?slugs=`: exactly the named flows (a slug that only contains a named one does not match, an unknown one is absent), and a list of nothing but non-slug values matching nothing
...
- `tests/e2e/list-items.spec.ts` — list pages draw CardShell items (`data-slotted`, a title, a meta line, more than one line): a note shows its key, its text under it and its tags, and Enter on its focused title opens it; a page of notes in a tall window fills the list's box in the list view and in the three-column grid, leaving less than one item (row) of room below the last, and switching the view on page 2 starts again from page 1; an artifact shows its name as the title and its actions carry hints; an execution shows its status in words and Enter on its title opens the run; on the flow list, whose page 2 items are taller than page 1's, Next sends one request and stays on page 2; on the audit log, whose page size settles in two overlapping requests, the older answer held until the newer one has answered does not replace the newer page; the administrator's users, audit log and workflows lists draw slotted items
- `tests/e2e/workflow-list-items.spec.ts` — a flow in the list reads as one CardShell item: the description under the name over more than one line, owner and visibility in words on the meta line, no status badge for a valid flow, the whole description on hover, the authored "when to pick it" only on a recommended universal flow, delete only on the reader's own flows, a click opening the flow; the Mine tab lists only the reader's flows and the Catalog tab only others'; the tabs' `aria-controls` names the list, and the status filter offers All, Valid, Invalid, Not checked; under name sort page 2 starts where page 1 ended, shares no flow id with it and is what the list shows, in name order
...
- `tests/workflow/scenarios/beginner-examples.test.ts` — the six learning examples (three levels, English and Russian): zero validation errors and no process diagnostics, no template in any text the agent receives, no back-edge, no global variable or expression, every fork reading one answer of the asking step, the two languages of a level structurally identical, and every branch of every flow running to its own end in both languages

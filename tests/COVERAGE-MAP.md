...
- `tests/unit/web-frontend/run-canvas-geometry.test.tsx` — the map's geometry between two ports once the layout has chosen the lanes: a lane edge leaving its port sideways into its own column, travelling the lane and coming back in beside its target, an outer rank running further from the card on both sides, a forward step straight between ports at one height and an elbow through the laid gap otherwise, a self transition dipping under the bottom port; the stacked preset keeping the laid lane while leaving the right port and entering the left one (and turning once halfway when the lane has no waypoints); and `portRanks` giving the edges arriving from above and from below disjoint column ranges, the nearer lane inside and the port that travels furthest outermost, with forward edges and self-loops taking no column
...
- `tests/unit/web-frontend/run-layout.test.ts` — deterministic non-overlapping ELK canvas layout of every annotated bundled flow, with the process rows (sequence, branches, hubs and loop-only blocks beneath), the balanced and stacked presets, the ported card's height estimate and no edge through a block
- `tests/unit/web-frontend/run-map-view.test.tsx` — the map's contents list (index badge, status, counts, typical time), ported cards with their run facts and bound list, the stored minimap flag and the phone column
...
- `tests/unit/web-frontend/run-canvas-geometry.test.tsx` — the map's port-to-lane geometry: lane edges in columns of their own, the stacked preset's side ports, self-loop dips
...
- `tests/e2e/workflow-canvas-controls.spec.ts` — the controls of a process diagram live in one toolbar above it on both views and the old floating React Flow cluster is gone: zoom in and zoom out move the camera's scale each way, fit gives the overview back (the whole graph inside its box; on the map the readable floor anchored at the first block), the navigator appears and disappears with its toolbar button and the choice survives a reload, and the step finder is folded away until asked for and then selects what it finds (the owning block on the map, the step's own card on the graph)

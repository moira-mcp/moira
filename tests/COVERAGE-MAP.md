...
- `tests/unit/web-frontend/steps-view-model.test.ts` — the steps view's model on the shipped learning examples: numbered instructions joined by unlabelled arrows on a straight sequence, the fork's authored answer labels in English and in Russian, one finish per path, a loop as a single return to the open step with unreachable teleports left out, and nothing to read without a start node; `humanizeVariable` reading variables as words; the recommendation registry offering the examples in the interface language before the universal flows and opening only the system's examples on the steps view

...
- `tests/unit/shared/workspace-resource-policy.test.ts` — disabled safe defaults, finite lifecycle/operation/transfer ceilings, absence of any per-day operation budget including an ignored removed variable, unit conversion, shipped multi-workspace active ceilings with the instance ceiling above the per-user one, a retained-output ceiling far above the response payload bound that fails closed when configured below it, a background-command ceiling above the bounded-command ceiling, and fail-closed malformed or inconsistent resource and aggregate transfer limits

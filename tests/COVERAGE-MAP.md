...
- `tests/e2e/execution-inspector-ux.spec.ts` — the run page keeps the inspector's toolbar (id copy, status, refresh), the context tab with its fullscreen editor, the errors/steps/locks tabs, the technical node graph one click away with the current-node focus moving the viewport off the overview, the panel beside the run on desktop and under it on a phone; the admin variant (owner info, waiting process view, read-only context, answer form) on instances with the multi-user admin capability
...
- `tests/unit/web-frontend/run-layout.test.ts` — canvas layout of the six annotated bundled flows through ELK: no overlapping blocks, every transition drawn once as an edge or a hub exit chip, forward edges never pointing backwards, deterministic placement, and hubs after their sources
...
- `tests/e2e/flow-page.spec.ts` — the flow page replacing the workflow detail page (see the execution domain row for the full scope); the kept workflow-page specs below exercise its graph mode and owner actions unchanged

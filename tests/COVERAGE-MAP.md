...
- `tests/unit/shared/workspace-resource-policy.test.ts` — disabled safe defaults, finite lifecycle/operation/transfer ceilings, absence of any per-day operation budget including an ignored removed variable, unit conversion, shipped multi-workspace active ceilings with the instance ceiling above the per-user one, and fail-closed malformed or inconsistent resource and aggregate transfer limits

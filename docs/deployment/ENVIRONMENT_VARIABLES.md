...
- `MARKETPLACE_PUBLIC_URL` - the public hosted store that self-host instances promote (a "Public store" nav link + the workflows-home catalog link). Fallback: `https://moira-mcp.com`. The promotion is **orthogonal to `MARKETPLACE_ENABLED`** — it is gated only by topology: shown whenever this instance's origin differs from this URL (so every self-host install promotes it in both flag states) and suppressed on the store itself. A scheme-less value is normalized to `https://`. Suppression is by **exact origin** (scheme + host + port), so the canonical store deployment must set `MARKETPLACE_PUBLIC_URL` to its own exact public origin (or rely on the default) to avoid self-promotion. Getters: `getMarketplacePublicUrl()` / `isPublicStorePromotionEnabled()` (`packages/shared/src/config/env.ts`).
